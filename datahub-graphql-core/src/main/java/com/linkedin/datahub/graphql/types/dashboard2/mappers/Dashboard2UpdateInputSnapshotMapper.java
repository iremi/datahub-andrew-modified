...
public class Dashboard2UpdateInputSnapshotMapper implements
        InputModelMapper<Dashboard2UpdateInput, Dashboard2Snapshot, Urn> {
    ...
    public Dashboard2Snapshot apply(@Nonnull final Dashboard2UpdateInput dashboard2UpdateInput,
        ...
        if (dashboard2UpdateInput.getTags() != null || dashboard2UpdateInput.getGlobalTags() != null) {
            ...
            if (dashboard2UpdateInput.getGlobalTags() != null) {
                ...
                globalTags.setTags(
                        new TagAssociationArray(
                                dashboard2UpdateInput.getTags().getTags().stream().map(
                                        element -> TagAssociationUpdateMapper.map(element)
                                ).collect(Collectors.toList())
                        )
                );
