...
public class GmsGraphQLEngine {
    ...
    /**
     * Configures resolvers responsible for resolving the {@link com.linkedin.datahub.graphql.generated.Dashboard2} type.
     */
    private void configureDashboard2Resolvers(final RuntimeWiring.Builder builder) {
        builder.type("Dashboard2", typeWiring -> typeWiring
                .dataFetcher("relationships", new AuthenticatedResolver<>(
                        new EntityRelationshipsResultResolver(graphClient)
                ))
        );
        builder.type("Dashboard2Info", typeWiring -> typeWiring
                .dataFetcher("charts", new AuthenticatedResolver<>(
                        new LoadableTypeBatchResolver<>(chartType,
                                (env) -> ((Dashboard2Info) env.getSource()).getCharts().stream()
                                        .map(Chart::getUrn)
                                        .collect(Collectors.toList())))
                )
        );
